...
from core.optimizer import grid_search_all_assets
...
class AllAssetsOptimizerWindow:
    """
    Optimize engine-level envelope across all assets for a timeframe + strategy.
    """
    ...
    def __init__(self, gui: Any):
        ...
        self.win.title("Optimize All Assets (timeframe)")
        ...
        content.rowconfigure(4, weight=1)  # results row

        # Defaults from main GUI
        default_tf = gui.timeframe_var.get()
        default_strategy = gui.strategy_var.get()
        default_mode = gui.mode_var.get()
        default_use_router = gui.use_router_var.get()
        tf_values = gui._get_available_timeframes()

        # ------------------------------------------------------------------ #
        # HEADER: timeframe / strategy / mode / router / RUN BUTTON
        # ------------------------------------------------------------------ #
        ...
        ttk.Label(header, text="Timeframe:").grid(
            row=0, column=0, sticky="w", padx=(0, 8), pady=(0, 2)
        )
        self.tf_var = tk.StringVar(
            value=default_tf if default_tf in tf_values else (tf_values[0] if tf_values else "1h")
        )
        ...
        self.tf_combo.grid(row=0, column=1, sticky="w", padx=(0, 8), pady=(0, 2))

        # Run button pinned in header
        self.run_button = ttk.Button(
            header,
            text="Run All-Assets Optimization",
    ...
    def run_optimization(self) -> None:
        ...
        self.status_label.config(text="Running all-assets optimization…")
        ...
        try:
            ...
            if not tf:
                ...
                self.status_label.config(text="Error: missing timeframe")
                return
            ...
            if df_res is None or df_res.empty:
                ...
                self.status_label.config(
                    text="All-assets optimization completed. No results."
                )
                return
            ...
            df_show = summarize_with_region(
                ...
                status_prefix="All-assets optimization completed",
            )
            ...
            self.results_text.insert(
                "1.0", "All-assets optimization failed:\n\n" + traceback.format_exc()
            )
            self.status_label.config(text="All-assets optimization failed.")
        finally:
...
def open_window(gui: Any) -> None:
    ...
    """
    AllAssetsOptimizerWindow(gui).show()


# gui/optimizer_envelope_all.py v0.3 (343 lines)
