# backtester.py
# FULL FILE — PHASE 4: risk info in summary
...
class BacktesterGUI:
    ...
    def load_config(self):
        default = {
            "data_file": "", "timeframe": "1h", "strategy": "", "mode": "balanced",
            "candles": 0, "show_equity": True, "run_mode": "Single",
            "use_router": False,
    ...
    def setup_styles(self):
        ...
        style.configure('TButton', padding=10, font=('Consolas', 18, 'bold'))
        ...
        style.configure('TCheckbutton', background='#0d1117',
                        foreground='#c9d1d9', font=('Consolas', 18))
    ...
    def create_widgets(self):
        left = ttk.Frame(self.root, padding="30")
        left.grid(row=0, column=0, sticky="ns", padx=25, pady=25)
        self.root.grid_columnconfigure(0, weight=1)
        self.root.grid_rowconfigure(0, weight=1)

        ttk.Label(left, text="REGIME BACKTESTER",
                  font=('Consolas', 24, 'bold'), foreground='#58a6ff') \
            .grid(row=0, column=0, columnspan=3, pady=(0, 30))
        ...
        ttk.Combobox(left, textvariable=self.timeframe_var,
            ...
            .grid(row=1, column=1, columnspan=2, sticky="ew", pady=(0,15))
        ...
        self.file_combo = ttk.Combobox(left, textvariable=self.file_var,
                                       state="readonly")
        self.file_combo.grid(row=2, column=1, columnspan=2, sticky="ew", pady=(0,15))
        ...
        self.strategy_combo = ttk.Combobox(left, textvariable=self.strategy_var,
                                           state="readonly")
        self.strategy_combo.grid(row=3, column=1, columnspan=2, sticky="ew", pady=(0,15))

        self.router_frame = ttk.LabelFrame(left, text="REGIME ROUTER", padding=10)
        self.router_frame.grid(row=4, column=0, columnspan=3, sticky="ew", pady=(10,0))
        ...
        ttk.Label(left, text="MODE").grid(row=5, column=0, sticky="w", pady=(0,5))
        self.mode_var = tk.StringVar(value=self.config.get("mode", "balanced"))
        ttk.Combobox(left, textvariable=self.mode_var,
                     values=["conservative", "balanced", "aggressive"],
                     state="readonly") \
            .grid(row=5, column=1, columnspan=2, sticky="ew", pady=(0,15))
        ...
        self.run_mode_combo = ttk.Combobox(left, textvariable=self.run_mode_var,
                     values=["Single", "All Strategies", "All Assets"],
                     state="readonly")
        self.run_mode_combo.grid(row=6, column=1, columnspan=2, sticky="ew", pady=(0,15))
        ...
        ttk.Entry(left, textvariable=self.candles_var) \
            .grid(row=7, column=1, columnspan=2, sticky="ew", pady=(0,15))
        ...
        ttk.Checkbutton(left, text="Show Equity Curve",
            ...
            .grid(row=8, column=0, columnspan=3, sticky="w", pady=(0,15))
        ...
        ttk.Checkbutton(left, text="Use Regime Router",
            ...
            .grid(row=9, column=0, columnspan=3, sticky="w", pady=(0,15))

        ttk.Button(left, text="RUN BACKTEST", command=self.run_backtest) \
            .grid(row=10, column=0, columnspan=3, pady=(0,30))

        right = ttk.Frame(self.root, padding="30")
        right.grid(row=0, column=1, sticky="nsew", padx=25, pady=25)
        self.root.grid_columnconfigure(1, weight=3)

        self.output_text = scrolledtext.ScrolledText(right, wrap=tk.WORD,
                                                     bg="#0d1117", fg="#c9d1d9",
                                                     font=('Consolas', 16))
        self.output_text.grid(row=0, column=0, columnspan=2, sticky="nsew", pady=(0,15))
    ...
    def _run_single(self, mode, use_router, max_c, mappings):
        ...
        try:
            summary, result = run_backtest(df, mode, strat,
    ...
    def _run_all_strategies(self, mode, use_router, max_c, mappings):
        ...
        for s in list_strategies():
            try:
                summ, _ = run_backtest(df, mode, s, use_router=False)
    ...
    def _run_all_assets(self, mode, use_router, max_c, tf, mappings):
        ...
        for pair, data in manifest["pairs"].items():
            ...
            try:
                ...
                summ, result = run_backtest(df, mode, strat,
                                            use_router=use_router,
                                            strategy_mappings=mappings)
                result.asset = pair
                result.save_trades(RESULTS_DIR)
